...
-- Consultas com WHERE NOT

-- Selecionar alunos que não estão em cursos com códigos 2 e 4
SELECT NomeAluno
FROM Alunos
WHERE Curso NOT IN (2, 4);

-- Selecionar notas de alunos cujos códigos não são 1 e 3
SELECT Alunos.NomeAluno, Notas.Nota
FROM Alunos
JOIN Notas ON Alunos.CodigoAluno = Notas.CodigoAluno
WHERE Alunos.CodigoAluno NOT IN (1, 3);

-- Selecionar cursos que não são os de códigos 1 e 5
SELECT NomeCurso
FROM Cursos
WHERE CodigoCurso NOT IN (1, 5);
